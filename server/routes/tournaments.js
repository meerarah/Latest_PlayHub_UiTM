...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query(
      'INSERT INTO Tournaments (name, sport, date, time, venue, maxTeams, description, status, tournamentRequestID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [name, sport, normalizedDate, time || '09:00 AM', venue || 'TBA', maxTeams || 8, description || '', 'active', requestId || null]
    );
